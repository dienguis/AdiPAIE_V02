...
namespace AdiPAIE_V02.Module.Services
{
    ...
    ///   - Pied de page avec espace de signature formateur
    ///
    /// La conversion HTML → PDF utilise LibreOffice headless (même pattern
    /// que AttestationFormationService). Si LibreOffice est absent, le HTML
    /// est retourné directement (mode dégradé).
    /// </summary>
    public static class FeuillEmargementService
    {
        ...
        // ─────────────────────────────────────────────────────────────────
        /// <summary>Génère la feuille d'émargement et la retourne en bytes (PDF ou HTML).</summary>
        ...
        private static string BuildHtml(SessionFormation session, Company company)
        {
            ...
            foreach (var insc in participants)
            {
                ...
                lignesHtml.Append(
                    ...
                    "<td class='sig'></td>" +  // colonne signature vide
                    "</tr>");
            }

            // Ajouter des lignes vides si moins de 5 participants (minimum visuel)
            for (int i = participants.Count; i < 5; i++)
            {
                lignesHtml.Append(
                    "<tr><td class='c'>&nbsp;</td><td>&nbsp;</td>" +
                    "<td class='c'>&nbsp;</td><td>&nbsp;</td><td class='sig'>&nbsp;</td></tr>");
            }

            // ── Infos session ─────────────────────────────────────────
            var datesPeriode = session.DateDebut.Date == session.DateFin.Date
                ? session.DateDebut.ToString("dd MMMM yyyy", Fr)
                : $"du {session.DateDebut:dd MMMM yyyy} au {session.DateFin:dd MMMM yyyy}";
...
<head>
  ...
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .info-label {{ color: #555; white-space: nowrap; min-width: 90px; font-weight: bold; }}
    /* Tableau */
    ...
    .sign-box {{ border: 1px solid #999; border-radius: 4px; padding: 10px 16px;
